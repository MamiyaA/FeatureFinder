...
#plot all the raw turnover rate and one for the location.
#st.subheader("Current turnover rate at each location")
fig1, ax1 = plt.subplots()
sb.scatterplot(data=TurnOver2*100, ax = ax1)
ax1.set_ylabel('turnover rate (%)')
ax1.set_title('current turnover rate at each location')
#just for the current location
sb.scatterplot(data=TurnOver2[index]*100, color=sb.xkcd_rgb["pale red"], sizes = 15, ax = ax1)
plt.tick_params(
...
st.subheader("Q55: Target score to achieve the turnover goal")
#need to check if we reached the limit (score must be between 0 and 1)
TargetScore = round(new_feature_before_transform.iloc[0], 2 - int(np.floor(np.log10(abs(new_feature_before_transform.iloc[0])))))
if TargetScore >= 0 and TargetScore <= 1:
   st.write(TargetScore)
elif TargetScore < 0:
   st.write('0: reached score limit')
elif TargetScore > 1:
   st.write('1: reached score limit')

st.markdown('***')
#have option for other features as well. First get it working for the 2nd question because the transformation is the same.
...
st.subheader("Current score for the (Q21: Managers avoid playing favorites)")
st.write(round(current_feature2.iloc[0], 2 - int(np.floor(np.log10(abs(current_feature2.iloc[0]))))))
st.subheader("Q21: Target score to achieve the turnover goal")
#need to check if we reached the limit (score must be between 0 and 1)
TargetScore2 = round(new_feature_before_transform2.iloc[0], 2 - int(np.floor(np.log10(abs(new_feature_before_transform2.iloc[0])))))
if TargetScore2 >= 0 and TargetScore2 <= 1:
   st.write(TargetScore2)
elif TargetScore2 < 0:
   st.write('0: reached score limit')
elif TargetScore2 > 1:
   st.write('1: reached score limit')
...
#st.write(round(new_feature_before_transform2.iloc[0], 2 - int(np.floor(np.log10(abs(new_feature_before_transform2.iloc[0]))))))
